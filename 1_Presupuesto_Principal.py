...
if 'start_calculation' not in st.session_state or not st.session_state.start_calculation:
    ...
    if st.session_state.aves_programadas <= 0 or st.session_state.peso_objetivo <= 0:
        ...
        try:
            ...
            tabla_filtrada = tabla_filtrada.iloc[:len(mortalidad_acum)]
            ...
            if kilos_totales_producidos > 0:
                ...
                tabla_filtrada['Costo_Kg_Dia'] = tabla_filtrada['Fase_Alimento'].map(costos_kg_map)
                ...
                costo_desperdicio_total += costo_otros_desperdiciados
                ...
                with col1_graf:
                    fig, ax = plt.subplots()
                    # ... (código del gráfico de crecimiento) ...
                    st.pyplot(fig)
